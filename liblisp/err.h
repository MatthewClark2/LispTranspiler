 ...
 * based on what the currently set error behavior is. Use `return raise(...)` in the same place you would use `raise_err E`.
 ...
 * @param cause the reason for the raise_err. If `None`, no exit will occur, but details will still be printed to `stderr`.
